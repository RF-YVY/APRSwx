// Node.js script to convert GRLevelX placefile to JSON
// Usage: node placefileToJson.js <input.placefile> [output.json]
...
// Browser usage: placefileToJson(placefileText)
export { placefileToJson };

// Node.js CLI usage: ts-node utils/placefileToJson.ts input.placefile output.json
if (require.main === module) {
  ...
  if (!inputFile) {
    console.error('Usage: ts-node utils/placefileToJson.ts <input.placefile> [output.json]');
    process.exit(1);
  }
  const content = fs.readFileSync(inputFile, 'utf8');
  const strikes = placefileToJson(content);
  if (outputFile) {
    fs.writeFileSync(outputFile, JSON.stringify(strikes, null, 2), 'utf8');
    console.log(`Wrote ${strikes.length} strikes to ${outputFile}`);
  } else {
    console.log(JSON.stringify(strikes, null, 2));
  }
}
